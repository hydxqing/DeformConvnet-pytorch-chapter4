...
class EDABlock(nn.Module):
    def __init__(self,ninput, dilated, k = 40,dropprob = 0.02):
        ...
        self.offset2 = ConvOffset2D(filters=k)#kernel_size=(3, 1),stride=1, padding=(dilated,0), dilation = dilated
    ...
    def forward(self, x):
        ...
        #print 'conv1x1',output.size()
        output = self.offset1(output)
        output = self.conv3x1_1(output)
        output = self.conv1x3_1(output)
        output = self.bn1(output)
        output = F.relu(output)

        output = self.offset2(output)
